...
config['best_of_categories'] = config['best_of_categories'].replace(' ', '').split(',')
...
def generate_best_of_lists(r):
    ...
    submission = r.get_submission(submission_id = config['best_of_nomination_post_id'])
    ...
    for nomination in nominations:
        ...
        for category in config['best_of_categories']:
            ...
            if category in body:
                ...
                entries['categorized'][category].add((entry.title, entry.permalink, entry.url, category))

        if has_no_category:
            entries['uncategorized'].add((entry.title, entry.permalink, entry.url))

    with open('best_of_{0}_categorized.txt'.format(config['best_of_nomination_post_id']), 'w') as f:
        for category in sorted(entries['categorized'].keys()):
            f.write('>>>{0}\n'.format(category))
            for entry in entries['categorized'][category]:
                f.write('* [{0}]({1}) - [Photo]({2})\n'.format(entry[0], entry[1], entry[2]))

    with open('best_of_{0}_uncategorized.txt'.format(config['best_of_nomination_post_id']), 'w') as f:
        for entry in entries['uncategorized']:
            f.write('* [{0}]({1}) - [Photo]({2})\n'.format(entry[0], entry[1], entry[2]))

    print 'Aggregation of {0} nomination thread completed'.format(config['best_of_nomination_post_id'])


def post_nominations_to_voting_thread(r):
    voting_thread = r.get_submission(submission_id = config['best_of_voting_post_id'])
    target_parent = ''
    with open('best_of_{0}_categorized.txt'.format(config['best_of_nomination_post_id']), 'r') as f:
        for line in f:
            if '>>>' in line:
    ...
    print 'Population of {0} voting thread completed'.format(config['best_of_voting_post_id'])
